...
import parkMap from "@/assets/park_map.png";
import parkNormalMap from "@/assets/park_normal_map.png";
import Cones from "./cone";
import ParkPillar from "./parkPillar";
...
export default function Base(props: BaseProps) {
  ...
  const projection = useMemo(() => {
    const center =
      outlineData?.features[0]?.properties.centroid ??
      data.features[0].properties.centroid;

    // 园区范围远小于省域，放大投影尺度以贴近 Demo2 场景尺寸
    return geoMercator().center(center).scale(6000).translate([0, 0]);
  }, [data, outlineData]);
